...
- 当前阶段：阶段 0；0.1 待验证，0.4 进行中，0.5 已完成
- 下一步：完成 0.1 理解验收与 0.4 数据/计费边界确认
...
| npm | 可用 | `11.6.2` |
| Git | 可用 | `2.50.1` |
| Pi | 已安装并启动 | `/opt/homebrew/bin/pi`，版本 `0.82.1`；实际终端 TUI 启动成功 |
...
1. 讲解：先建立概念模型和安全边界。
2. 演示：我展示最小操作或最小实现。
3. 实操：由用户亲自执行核心步骤，不由我代做验收题。
4. 检查：核对命令输出、文件差异、会话记录或测试结果。
5. 纠错：说明错误原因，并要求用户完成一次正确重试。
6. 验收：满足本计划写明的证据后，立即勾选对应事项。
...
- 模型认证、付费调用、外部发布和安装第三方 Package 前必须单独确认。
...
- [ ] 0.1 完成本机环境体检
  - 待验证：环境报告已生成，等待用户回答四项理解题。
  - 证据：`docs/learning/00-environment-report.md`
  - 内容：Node.js、npm、Git、终端、Pi 安装状态和学习仓库隔离。
  - 验收：形成一份不含敏感信息的环境报告，并能说明每项依赖的作用。
- [ ] 0.2 建立 Pi 架构心智模型
...
- [ ] 0.4 选择认证与费用方案
  - 进行中：学习期 Provider 已选择第三方 OpenAI 兼容中转站，认证材料为 Base URL 与 API Key。
  - 已确认：Base URL、Responses 协议、`openai/gpt-5.6-sol`、凭据保存及首次实际调用均已验证；`auth.json` 权限为 `0600`。
  - 待确认：中转站的数据处理规则、计费规则和后续调用费用边界。
  - 下一步：记录数据流向、计费方式及学习期间允许的费用边界。
...
- 状态：进行中（0.4 数据/计费边界）；0.1 待验证，0.5 已完成。
- 已完成：Pi CLI、`fd`、中转配置、凭据权限、`openai/gpt-5.6-sol` 首次响应、Session 保存及退出状态均已验证。
- 阻塞：无。
- 下一步：完成 0.1 理解验收和 0.4 数据/计费边界确认。
...
| 2026-07-29 | 0.1 本机环境体检 | 待验证 | `docs/learning/00-environment-report.md` | 用户回答四项理解题 |
| 2026-07-29 | 0.4 认证与费用方案 | 进行中 | 中转配置、API Key 保存、`openai/gpt-5.6-sol` 实际响应及凭据权限已验证 | 核对数据处理、计费规则和费用边界 |
