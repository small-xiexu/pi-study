...
假设你想写一个代码审查脚本：脚本把代码和要求交给 Pi，取得审查意见后，再整理成报告。这样的程序需要直接提交任务和读取结果，可以使用 Pi 的 SDK。

你的 Node.js 程序负责准备输入、接收结果和安排后续处理，Pi 继续负责模型调用、工具执行和会话。本篇用上一章的固定回复脚本完成一个最小示例，观察会话怎样创建、结果从哪里取得，以及结束后怎样清理。

## 把 Pi 嵌入程序，究竟接管了什么？

SDK 是供程序调用的公开接口。调用方决定什么时候提交任务、显示哪些内容、怎样判断业务成功，以及何时结束应用。Pi 提供已有的任务运行机制；你不需要为了嵌入它而重新写一次 Agent Loop。

使用 SDK 时，Node.js 代码可以直接传入对象、订阅事件并读取结构化状态。应用也要接手原本由交互界面处理的工作，明确怎样处理忙时输入、超时和退出。
...
`prompt()` 的返回值是 `Promise<void>`，作用是让程序等待这次调用收尾。中途进度通过事件到达，回答内容和结束状态则需要从本轮事件或会话状态中取得。

订阅会接收建立订阅之后的事件，因此应在任务开始前订阅，结束后取消订阅。需要查找先前的结果时，可以读取 Session 或当前状态。
...
只开放 `read` 还不等于只能读练习文件。如果应用要求限制读取范围，需要在实际执行前检查路径，再让内置 Executor 打开目标。路径检查和检查期间文件被替换的问题，在[任务台](26-构建任务台与异常测试.md)继续讲。
...
队列的 `one-at-a-time` 表示每个投递点取最早一条，`all` 表示取该队列当前全部消息；这不是并行运行多个 Prompt。`queue_update` 和待处理计数帮助观察入队与交付，尚未投递的文本不能当成模型已经收到。`clearQueue()` 清空并返回两类未处理消息，应用可留作草稿。设计取消操作时，还要决定排队消息是继续保留，还是取回给用户编辑。

## 5. 压缩、保存和恢复

`await session.compact(instructions)` 会生成摘要，真实模型模式下可能调用服务，磁盘中的原始历史仍会保留。恢复时用 `SessionManager.open(...)` 或 `continueRecent(...)` 显式选定记录，再交给新会话。

`SessionManager.inMemory()` 不保存会话文件；`SessionManager.create(cwd, sessionDir)` 创建持久会话。保存到磁盘前，还需要准备私有目录、限制同时写入的进程，并明确损坏文件的处理方式和输出脱敏规则。

会话记录可能包含 Prompt、工具结果和回答。程序日志应只输出必要的状态和诊断信息，避免直接打印 `session.state` 或原始异常。需要展示回答时，再按应用的输出规则处理正文。
