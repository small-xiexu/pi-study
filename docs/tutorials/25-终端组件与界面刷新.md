...
打开对话框后，焦点交给 BookDialog，再由它传给内部 Input。输入“小”时，Input 保存文字和光标，外层请求刷新；`render(width)` 生成当前文字行，Container 组合标题与输入框，TUI 合成 Overlay 并把变化写到终端。这一步没有提交模型任务。
...
按 Return 时，Input 的提交回调将文本交给 `done`，Overlay 关闭，焦点恢复到原组件；按 Esc 则交回取消值。下一次打开创建新实例。如果此时模型后台恰好也发来事件，它可以独立更新消息区：显示一个对话框并没有暂停 Agent，关闭对话框也没有取消 Agent。
