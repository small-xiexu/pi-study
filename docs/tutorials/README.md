...
Pi 是运行在终端里的编程助手：它准备材料、接收模型提出的工具申请、安排工具执行、把结果交回模型，并保存记录、展示进度。这套教程沿一次任务的运行过程，介绍各部分怎样配合，以及怎样配置、使用和扩展 Pi。

这套教程适合会基本编程、尚未接触 Pi 的读者。“小书架”是贯穿其中的书单管理应用，用来记录书名和阅读进度；各篇围绕它的统计修复、代码审查和开发工具展开。Pi 则是协助处理这些任务的编程助手，后面的主题、扩展和终端组件都是对 Pi 的定制。

每篇先说明要解决的问题，再介绍操作、原理和示例。首次阅读可以先看完整过程，需要动手时再准备对应环境，不必一次安装所有依赖或调用所有服务。正文会解释所需知识，实验目录提供进一步核对的代码和运行说明。
...
图中展示了各部分的主要职责。中间的回环表示模型提出下一步操作，Pi 安排执行，再把结果交回模型。阅读后续章节时，可以沿着这个过程找到每项功能的位置。
...
这张表用于快速查阅。Context、Runtime、Model 等词在不同接口中可能指不同对象，具体含义需要结合所在模块及版本查看。
