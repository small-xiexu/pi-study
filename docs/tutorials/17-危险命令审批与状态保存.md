...
你希望 Pi 在执行某类 Shell 命令前先询问，只有你明确同意才继续。这需要扩展在执行前拦截请求，并按 Pi 的要求返回允许或拒绝结果。

本篇用两条只打印文字的命令演示审批，便于观察放行和拒绝。随后再做一个独立的计数示例：每次人工检查后输入 `/book-mark` 登记一次，切换会话分支或重载扩展时，恢复当前分支的登记次数。两个例子分别说明执行前控制和会话状态保存。
...
本例让两种入口共用决策函数，再各自返回宿主认可的拒绝格式。检查范围限于模型工具和用户 Shell 这两条入口；扩展自行启动的进程等其他执行方式，需要另行控制。进程本身仍继承当前用户的系统权限。
...
以人工检查计数为例：你在方案 A 中检查并登记了两次，回到共同起点讨论方案 B 后，又登记一次。如果把所有操作都加到一个全局计数器上，就会显示 3；而你想看到的是当前方案 B 的 1 次。只用内存变量保存也不够，重载扩展后这个数字还可能变回 0。
...
这个计数表示通过 `/book-mark` 成功登记的人工检查次数。它不统计实际工具执行，也不参与命令审批。
...
本例只接受两条固定命令：一条可以直接打印文字，另一条需要确认后才能打印，其他命令一律拒绝。两条命令本身都不写文件，适合用来观察审批流程。
...
仓库已有的 Shell 状态还区分允许、拒绝及原因，并使用完整快照恢复。可从 [shell-state.ts](../../.pi/extensions/pi-study-guard/shell-state.ts) 和 [shell-widget.ts](../../.pi/extensions/pi-study-guard/shell-widget.ts) 查看这些状态怎样保存并显示。
...
审批过程发生异常时，`catch` 会返回拒绝结果。`user_bash` 需要按接口要求返回替代结果，仅抛出异常无法保证宿主停止执行命令。
...
下面实现前面介绍的人工检查计数：通过 `/book-mark` 登记，重载或切换分支时恢复当前路线的计数。创建 `.pi/extensions/book-count.ts`：
...
代码按 Pi `0.85.1` 编写，只演示 macOS/Bash 下两条固定命令的审批。Windows PowerShell、其他扩展组合、真实进程停止和系统隔离需要分别验证。
