...
你在 Pi 终端里阅读回答和代码差异，觉得当前配色不够清楚，想换成更适合自己的颜色。这里要调整的是 Pi 的终端界面。保存主题文件后，还需要让 Pi 找到并选中它，界面才会发生变化。

本篇先介绍资源怎样加载，再用一个自定义主题演示选择和更新。这个过程也能帮助你排查模板、Skill 和扩展保存后没有生效的问题。
...
同一条错误可能出现在通知、工具结果或其他界面中。主题用 `error` 统一指定错误颜色，让不同位置的错误信息保持一致；更换主题时，也可以一起调整这些颜色。
...
主题按内容用途指定颜色。例如 `error` 用于错误，`toolDiffAdded` 用于新增内容，`thinkingHigh` 用于高思考档位边框；这些内容出现在屏幕上的不同位置时，仍使用对应颜色。
...
| 操作 | 主要影响 | 使用时机 |
|---|---|---|
| `/reload` | 重读资源，更新扩展等运行实例 | 让刚保存的资源被发现或重新加载 |
| 退出后重新启动 | 建立新进程，重新解析启动环境 | 修改了启动环境，或需要重新建立进程 |
| `/new` | 开始新会话 | 开始另一段独立讨论 |
| 当前主题热更新 | 更新显示资源 | 调整当前已激活主题的配色 |
...
在 Pi 输入 `/settings`，选择 `dark` 或 `light`，观察文字、背景和选中项的配色是否适合阅读。
...
上一篇：[提示词模板与技能](13-提示词模板与技能.md)。下一篇：[第一个扩展与生命周期](15-第一个扩展与生命周期.md)。
