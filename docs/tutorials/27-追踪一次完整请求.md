...
这不是两个独立任务，也不是模型第一次回答失败。它是一次任务利用反馈逐步获得事实的正常过程。同一 Turn 还可以包含多个工具申请，因此不要把“一次用户提问”“一次模型生成”“一次工具执行”计成相同次数。

### 为什么回执必须保留原调用标识？

假设同时申请读取两个文件，只有两段文字而没有对应关系，模型就可能把 A 的结果当作 B。Tool Call ID 把回执绑定到原申请；名称、参数和错误标志则说明这份事实究竟来自什么操作。

完整反馈不仅要有内容，还要保留身份和结果性质。把拒绝转换成一条普通成功文本，或者把一个调用的结果接到另一个调用上，都会破坏后续判断，即使屏幕上能看到 marker。

### 为什么会有多份消息表示？

下一轮生成需要工作材料，外部订阅者需要运行通知，Session 需要可恢复的历史，TUI 需要适合显示的状态。它们服务于不同消费者，不能假定“写入了其中一处”会自动更新其余所有位置。

下面的时序图先保持完整主线，再逐层解释入口、协议、工具和回填。读每一步时同时问：控制交给了谁，这次交付的数据又到了哪里？
