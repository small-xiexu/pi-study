...
你已有一个经过审查的模型服务，它支持 Chat Completions，但不在 Pi 的内置目录里。此时先用配置描述它，不必马上实现一套 Provider。
...
这里的零价格只是虚构测试元数据，不说明任何真实服务免费；8192 和 1024 也只是练习值，不能靠改配置给远端模型增加能力。
...
协议会决定地址后缀、消息角色、工具字段和流式响应解释方式。把支持 Chat Completions 的服务标成 Responses，不是“更新的写法”，而是换了一份合同。
...
常见差异还包括 `max_tokens` 与 `max_completion_tokens`、工具结果是否要带名称，以及流是否提供结束原因。缺少结束原因不能随意当作成功 EOF，除非协议合同明确且使用对应支持选项。
...
本篇不新增旧式注册代码，只在这里提醒已有扩展的行为不同。未知 `modelOverrides` ID 可能被忽略，因此文件合法不代表你命中了目标模型。
...
> **模型配置描述已有能力，不创造能力；先检查结构，再验证请求，最后验证工具与失败合同。**

本篇按 Pi `0.85.1` 核对，虚构地址不提供真实推理服务。特殊认证和非标准协议放在下一篇处理。

上一篇：[模型接入与认证](19-模型接入与认证.md)。下一篇：[自定义服务商扩展](21-自定义服务商扩展.md)。
