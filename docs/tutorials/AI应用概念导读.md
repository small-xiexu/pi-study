...
| 通用概念 | Pi 中怎样理解 | 后面去哪看 |
|---|---|---|
| Model 与 Provider | Pi 选择模型并通过接入层联系服务，不自带替你推理的大模型权重 | [05 模型与用量](05-模型选择与用量.md)、[19 模型接入](19-模型接入与认证.md) |
| Harness 与 Agent Loop | Pi 定位为终端编程 Harness，组织模型、工具、反馈和会话 | [00 认识 Pi](00-安装与第一次对话.md)、[27 完整请求](27-追踪一次完整请求.md) |
| Context、Session、Compaction | 从历史和资源组织当前输入，保存会话并在需要时压缩 | [10 会话](10-保存恢复与会话分支.md)、[11 上下文](11-会话记录与上下文压缩.md) |
| Tool Calling | 默认基础工具为 read、write、edit、bash，可定制工具与检查 | [03 工具与边界](03-基础工具与操作边界.md)、[15 自定义工具](15-自定义工具与命令.md) |
| Rules、Templates、Skills | 项目约定与按需使用的方法资料，不是 OS 权限 | [08 项目规则](08-项目配置与规则.md)、[12 模板与技能](12-提示词模板与技能.md) |
| Extension、Hook、Package | 扩展宿主行为，再将资源打包交付 | [14 扩展](14-第一个扩展与生命周期.md)、[18 打包](18-打包安装与资源管理.md) |
| SDK 与 Runtime | 让 Node.js 程序直接控制 Pi 的运行对象 | [22 SDK](22-使用SDK提交与控制任务.md) |
| RPC、CLI、TUI | 跨进程协议、命令行入口与终端界面是不同使用方式 | [07 一次性运行](07-一次性运行与结构化输出.md)、[23 RPC](23-RPC协议与Java客户端.md)、[24 终端界面](24-终端组件与界面刷新.md) |
...
第一次阅读能把这几类职责分开就够了。接着回到[Pi 系统地图](README.md#先看系统地图)，再读[00 安装与第一次对话](00-安装与第一次对话.md)；具体 API、配置和实验在对应章节展开。
