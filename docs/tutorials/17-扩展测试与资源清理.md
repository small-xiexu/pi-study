...
将上一节的 `study-gate.ts` 放在测试目录，并创建 `study-gate.test.mjs`。这里使用 Node `>=22.19.0` 的 TypeScript 类型擦除能力；门禁只有类型导入，没有需要另外安装的运行依赖。
...
初始化只负责注册；后台资源在会话开始或首次使用时创建。关闭逻辑要幂等，也就是调用两次不会重复释放出错。
...
本篇门禁测试针对 16 的 Pi `0.85.1` 示例，不升级仓库原扩展，也不宣称已验证所有进程和终端行为。

上一篇：[危险命令审批与状态保存](16-危险命令审批与状态保存.md)。下一篇：[打包安装与资源管理](18-打包安装与资源管理.md)。
