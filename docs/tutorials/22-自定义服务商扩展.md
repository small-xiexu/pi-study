...
假设你要让 Pi 通过一个新网关访问模型。这个网关使用特殊的登录方式，返回工具调用时的字段也与现有协议不同。仅填写地址和模型 ID，Pi 还无法正确处理请求，需要用自定义 Provider 补上认证或响应转换的代码。

本篇先说明接入代码需要处理哪些数据，再实现一个返回固定文字的脚本 Provider。用固定响应可以直接观察事件和最终消息怎样产生，便于理解接口；真实网关的网络通信和认证则需要另行实现与验证。

## Provider 为什么不只是包一层 fetch？

`fetch` 负责取得 HTTP 响应，不知道哪一段是文字、哪一段是工具参数，也不知道服务结束是正常完成还是额度耗尽。Agent Loop 需要这些含义才能决定执行工具、继续请求或停止。

接入层需要把服务返回的数据转换为 Pi 的消息和事件，并准确说明响应怎样结束。如果把网络断开当作正常结束，上层可能接受半个答案；如果工具参数还没收齐就返回 Tool Call，本地可能执行错误操作。

### 为什么要有统一消息和统一事件流？

不同服务可能使用不同字段名和事件名。若每个 Agent、界面和客户端都直接理解所有厂商格式，每接一家服务就要到处改代码。统一表示把变化集中在接入处：上层处理文字、思考、工具请求和终态，不必到处判断服务商。

适配时仍需按模型和服务实际支持的能力处理请求，并保留调用标识、内容类型和停止原因。上层会依据这些信息决定是否执行工具、继续生成或结束任务。
...
Provider 实现模型接入，可以通过 Extension 注册到 Pi，也可以交给 SDK 使用。扩展负责注册，Provider 负责处理具体的目录、认证和请求。
...
假设服务返回一个读取申请，内容块编号为 0，调用 ID 为 `read-1`，工具名为 `read`。下面分步说明接入层怎样收集参数、形成工具请求并返回结果：

| 到达阶段 | 当前已经知道的内容 | 接入层该做什么 |
|---|---|---|
| 工具块开始 | ID=`read-1`，名称=`read` | 为本块建立累积状态 |
| 第一段参数 | `{"path":"books` | 保留片段；尚不是完整参数，不执行 |
| 第二段参数 | `.mjs"}` | 拼成 `{"path":"books.mjs"}`，仍检查块和消息终态 |
| 块与 Assistant 正常以工具请求结束 | 完整名称、ID、参数，`stopReason=toolUse` | 交给 Agent 做名称、Schema、门禁和取消检查 |
| 本地工具完成 | `toolCallId=read-1` 的 Tool Result | 编码回下一次服务请求，不能换一个不匹配的 ID |
| 第二次响应正常结束 | 根据文件内容形成最终文字 | 输出最终 Assistant 和唯一终态 |

如果网络在第一段之后断开，应形成失败；即使两段已经能解析成 JSON，消息最后报告 `length`，也不能据此断言这是一份完整可靠的工具申请。反过来，`contentIndex=0` 只标识当前 Assistant 内的内容块，下一条 Assistant 又可从 0 开始，不能拿它替代跨消息的 Tool Call ID。

做离线适配测试时，第二次脚本响应应先断言输入含 `read-1` 的结果，再生成回答。如果脚本无条件返回“成功”，即使忘记回传工具结果，测试也可能全绿。第 29 篇用故障对照进一步展示这个问题。
...
动态刷新要遵守传入的取消信号和 `allowNetwork`。缓存可用于展示已知目录，但不能据此判断服务在线。普通 `getModels()` 应返回已知模型；需要联网更新时，使用负责刷新的入口。
...
例如某次服务报告的输入总量为 1,800，其中缓存命中 1,000；输出为 200，没有缓存写入。若该协议的输入总量包含缓存，归一化后是普通输入 800、缓存读取 1,000、输出 200，总量 2,000。不能把原始 1,800 与缓存 1,000 再加一次。

假设每百万 Token 的虚构价格分别为普通输入 2、缓存读取 0.2、输出 8 个货币单位，则显示费用为 `800×2/1,000,000 + 1,000×0.2/1,000,000 + 200×8/1,000,000 = 0.0034`。真实服务可能采用不同的报告口径，先按协议拆分，再按对应元数据计算；这张小票不是服务账单。

接入层要将上下文溢出报告为可识别的容量错误，Pi 才可能进行压缩和重试。`contextWindow` 描述模型上限，`reserveTokens` 为后续内容预留空间，适配器还可能按模型限制调整单次输出的 `maxTokens`。这些值应与服务支持的范围一致；压缩会产生新请求，也可能失败。
...
这个脚本 Provider 的认证接口也需要明确实现：`resolve()` 表示当前已可用，并返回空的认证材料，因此无需提供 Key。
...
上一篇：[自定义模型配置](21-自定义模型配置.md)。下一篇：[使用SDK提交与控制任务](23-使用SDK提交与控制任务.md)。
