...
- 采集时证据：已记录环境元数据、Pi CLI 安装、中转认证、首次模型会话及四项理解结论
- 本次采集未覆盖：API Key 内容读取、破坏性工具调用和课程其他实操
...
- 采集时的当前目录和 Git 根目录均为 `<repo-root>`。
- 远程仓库为 `https://github.com/small-xiexu/pi-study.git`。
- 采集时分支为 `main`，学习计划与本报告已经发布到 `origin/main`。
...
自动化体检进程报告 `TERM=dumb`，这只反映自动化命令执行通道。用户已在实际 zsh 终端成功显示 Pi TUI，说明基础交互界面可以启动；本次自动化体检未覆盖颜色、图片和全部键盘快捷键。
...
| Pi 能力边界 | 本次体检覆盖安装启动、模型响应、Session 保存和只读 Shell；未覆盖文件编辑写入、Session 恢复、Extension、MCP/Subagent 和安全控制。 |
